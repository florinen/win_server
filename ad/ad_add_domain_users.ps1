...
New-ADUser -Name $User -AccountPassword (ConvertTo-SecureString $AccountPassword -AsPlaintext -Force) -Description "Acirrustech Admin User" -ChangePasswordAtLogon:$False -CannotChangePassword:$True -PasswordNeverExpires:$True -Path "OU=$ChildOU,OU=$OrganizationalUnit,DC=$Domain,DC=$DomainEnding"  -Enabled $True
